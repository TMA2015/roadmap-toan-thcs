...
with sync_playwright() as p:
    ...
    exam_page.goto(BASE + "kien-thuc/25-tong-hop-on-thi-10/bo-de-luyen/", wait_until="networkidle")
    check(exam_page.locator(".exam-directory a").count() == 3, "three full written exams in capstone hub")
    shot(exam_page, "topic25-exams-hub-desktop.png")
    exam_page.goto(BASE + "kien-thuc/25-tong-hop-on-thi-10/de-luyen-02/", wait_until="networkidle")
    key = exam_page.locator(".exam-answer-key")
    check(key.count() == 1 and not key.evaluate("(el) => el.open"), "original answer key closed")
    key.locator("summary").click()
    check(not key.evaluate("(el) => el.open"), "no pre-submission solution reveal")
    check(exam_page.locator(".floating-ai-launcher").count() == 0, "no reading AI on timed exam")
    exam_page.locator(".exam-btn-primary").click()
    check("01:59" in exam_page.locator(".exam-workspace-timer").inner_text(), "120-minute countdown started")
    shot(exam_page, "topic25-exam02-running-desktop.png")
    exam_page.on("dialog", lambda dialog: dialog.accept())
    exam_page.get_by_role("button", name="Nộp bài / Kết thúc").click()
    check(key.evaluate("(el) => el.open"), "solution only revealed after submit")
    check(exam_page.locator(".exam-rubric-item").count() == 12, "step-wise partial-credit checklist")
    exam_page.locator(".exam-rubric-item input").first.check()
    check("0,75" in exam_page.locator(".exam-rubric-total").inner_text(), "score computed from checked rubric step")
    check(exam_page.locator(".exam-gap-links a").count() >= 1, "gaps lead to anchors or academic lessons")
    shot(exam_page, "topic25-exam02-self-mark-desktop.png")
    exam_page.close()
